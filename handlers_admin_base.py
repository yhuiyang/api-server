...
class GetBlobStoreUploadUrl(webapp2.RequestHandler):

    def get(self):

        # the url to post to blobstore is dynamically generated by GAE,
        # when blob upload completed, GAE will invoke the callback handler
        # registered on the success_path
        success_path = self.request.get('success_path')
        if success_path:
...
routes = [
    ...
    webapp2.Route(r'/blobstore/get_upload_url', GetBlobStoreUploadUrl),
]
